...
async function boot(cfg: BootCfg): Promise<void> {
  ...
  // Dev visual-check (?hubshot): the real way to get up-to-4 tracks at a station — build a separate
  // line (with its own train) from the station to each of several destinations. Frame the station to
  // confirm the platforms + trains.
  if (location.search.includes('hubshot') && startHome) {
    network.player.money = 24_000_000;
    network.goal = networthGoal(Number.MAX_SAFE_INTEGER, 99999); // don't auto-win mid-sim
    const home = startHome;
    const dests = network.stations
      .filter((s) => s !== home)
      .sort((a, b) => a.pos.distanceToSquared(home.pos) - b.pos.distanceToSquared(home.pos))
      .slice(0, 3);
